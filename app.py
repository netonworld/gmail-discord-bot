...
def extract_payment_info(email_content, subject):
    ...
    # Para emails de Binance, buscar patrones específicos
    if "binance" in subject.lower() or "binance" in email_content.lower():
        # Buscar el monto en formato "Amount: X.X USDT"
        amount_pattern = r'Amount:\s*(\d+\.?\d*)\s*([A-Z]+)'
        match = re.search(amount_pattern, email_content)
        if match:
            payment_info["amount"] = match.group(1)
            payment_info["currency"] = match.group(2)
            print(f"💵 Monto detectado (Binance): {payment_info['amount']} {payment_info['currency']}")
        
        # Buscar el remitente en formato "From: User-XXXX"
        from_pattern = r'From:\s*([^\s\n]+)'
        match = re.search(from_pattern, email_content)
        if match:
            payment_info["sender"] = match.group(1)
            print(f"👤 Remitente detectado (Binance): {payment_info['sender']}")
        
        payment_info["method"] = "Binance"
    else:
...
def send_provider_notification(email_data, payment_info, provider_id, provider_config):
    ...
    if payment_info["sender"]:
        embed["fields"].append({
            "name": "👤 De",
            "value": payment_info["sender"],
            "inline": True
        })
...
if __name__ == '__main__':
    ...
    # Detectar método de pago si no se detectó antes
